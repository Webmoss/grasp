<template>
  <div class="nft-view-row">
    <div class="token-details">
      ...
      <div v-if="token && token.token?.lastSale?.timestamp" class="nft-token-detail">
        <span class="grey-label">Last Sale Date</span>
        <span class="nft-token-detail-value">
          {{ moment(token.token.lastSale.timestamp).format("Do M MMMM, h:mm:ss a") }}
        </span>
      </div>
      <div
        v-if="token && token.token.lastSale && token.token.lastSale.feeBreakdown"
